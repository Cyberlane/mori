...
func TestLoadRejectsDuplicateIdentities(t *testing.T) {
	...
	writeFixture(t, path, `{"schema_version":2,"identity_scope":"content","normalization_version":10,"entries":[{"id":"same"},{"id":"same"}]}`)
...
func writeFixture(t *testing.T, path string, content string) {
	...
	if normalize.Version != 10 {
		t.Fatalf("test fixture version = %d, want 10", normalize.Version)
	}
}
