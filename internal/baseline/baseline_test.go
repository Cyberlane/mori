...
func TestLoadRejectsDuplicateIdentities(t *testing.T) {
	...
	writeFixture(t, path, `{"schema_version":2,"identity_scope":"content","normalization_version":4,"entries":[{"id":"same"},{"id":"same"}]}`)
...
func writeFixture(t *testing.T, path string, content string) {
	...
	if normalize.Version != 4 {
		t.Fatalf("test fixture version = %d, want 4", normalize.Version)
	}
}
