...
module.exports = grammar({
  ...
  conflicts: $ => [
    [$.statement, $._conditional_else_head],
  ...
  rules: {
    ...
    ...preprocIf('_in_initializer_list', $ => $._initializer_list_item),
    preproc_if_in_else_clause: $ => seq(preprocessor('if'), field('condition', $._preproc_expression), '\n', $.else_clause, preprocessor('endif')),
    preproc_ifdef_in_else_clause: $ => seq(choice(preprocessor('ifdef'), preprocessor('ifndef')), field('name', $.identifier), $.else_clause, preprocessor('endif')),
    ...
    macro_declaration: $ => choice(
      seq('static', alias(/[A-Z][A-Z0-9_]*_DEFINE/, $.identifier), $.argument_list, ';'),
      seq(alias('DT_INST_FOREACH_STATUS_OKAY', $.identifier), $.argument_list),
      seq(alias('ZMK_DISPLAY_WIDGET_LISTENER', $.identifier), '(', $.identifier, ',', $.type_descriptor, ',', $.identifier, ',', $.identifier, ')'),
    ),
    ...
    _conditional_else_head: $ => seq('if', field('condition', $.parenthesized_expression), field('consequence', $.compound_statement)),

    conditional_else_statement: $ => seq(alias($._conditional_else_head, $.if_statement), choice(alias($.preproc_if_in_else_clause, $.preproc_if), alias($.preproc_ifdef_in_else_clause, $.preproc_ifdef))),
    ...
    initializer_list: $ => seq(
      '{',
      repeat($._initializer_list_item),
      optional(choice($.initializer_pair, $.expression, $.initializer_list)),
      '}',
    ),

    _initializer_list_item: $ => choice(
      seq(choice($.initializer_pair, $.expression, $.initializer_list), ','),
