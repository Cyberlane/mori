...
static bool scan_automatic_semicolon(TSLexer *lexer, const bool *valid_symbols, bool *scanned_comment) {
    ...
    switch (lexer->lookahead) {
        ...
        case '>':
            return false;
        case '<':
            if (!valid_symbols[TYPE_MEMBER_AUTOMATIC_SEMICOLON]) return false;
            // A new generic call signature must have parameters after its type
            // parameters. Preserve multiline generic return types otherwise.
            {
                unsigned depth = 0;
                do {
                    if (lexer->lookahead == '\'' || lexer->lookahead == '"' || lexer->lookahead == '`') {
                        ...
                        while (lexer->lookahead && lexer->lookahead != quote) {
                            if (lexer->lookahead == '\\') { skip(lexer); if (!lexer->lookahead) return false; }
                            skip(lexer);
                        }
                    ...
                    if (lexer->lookahead == '/') {
                        bool comment = false;
                        if (!scan_whitespace_and_comments(lexer, &comment)) return false;
                        continue;
                    }
                    if (lexer->lookahead == '=') {
                        skip(lexer);
                ...
                bool comment = false;
                if (!scan_whitespace_and_comments(lexer, &comment) || lexer->lookahead != '(') return false;
                lexer->result_symbol = TYPE_MEMBER_AUTOMATIC_SEMICOLON;
                return true;
            }
        case '=':
        case '?':
        ...
        case 'i':
            if (valid_symbols[TYPE_MEMBER_AUTOMATIC_SEMICOLON] && !valid_symbols[LOGICAL_OR]) {
                lexer->result_symbol = TYPE_MEMBER_AUTOMATIC_SEMICOLON;
                return true;
            }
            skip(lexer);
...
static inline bool external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    ...
    if (valid_symbols[AUTOMATIC_SEMICOLON] || valid_symbols[FUNCTION_SIGNATURE_AUTOMATIC_SEMICOLON] || valid_symbols[TYPE_MEMBER_AUTOMATIC_SEMICOLON]) {
