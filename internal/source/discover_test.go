...
func TestDiscoverClassifiesAndOptionallyExcludesGeneratedFiles(t *testing.T) {
	...
	if len(included.Files) != 4 || len(included.Excluded) != 0 {
		t.Fatalf("included result = %#v, want four files", included)
	}
	generated := 0
	...
	if generated != 2 {
		t.Fatalf("generated files = %d, want 2: %#v", generated, included.Files)
	}

	excluded := Discover([]string{root}, Options{ExcludeGenerated: true})
	if len(excluded.Files) != 2 || len(excluded.Excluded) != 2 {
		t.Fatalf("excluded result = %#v, want two analyzed and two excluded", excluded)
	}
