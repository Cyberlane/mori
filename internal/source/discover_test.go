...
func TestDiscoverClassifiesAndOptionallyExcludesGeneratedFiles(t *testing.T) {
	...
	if len(included.Files) != 6 || len(included.Excluded) != 0 {
		t.Fatalf("included result = %#v, want six files", included)
	}
	generated := 0
	...
	if generated != 3 {
		t.Fatalf("generated files = %d, want 3: %#v", generated, included.Files)
	}

	excluded := Discover([]string{root}, Options{ExcludeGenerated: true})
	if len(excluded.Files) != 3 || len(excluded.Excluded) != 3 {
		t.Fatalf("excluded result = %#v, want three analyzed and three excluded", excluded)
	}
